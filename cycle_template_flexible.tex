...
{% for edge in edges %}
\draw[{% if direction == 1%}edge-end-with-arrow{% elif direction == 2 %}edge-start-with-arrow{% else %}edge{% endif %}{% if edge.color %},{{edge.color}}{% endif %}] ({{ edge.start }}:\radius) arc ({{ edge.start }}:{{ edge.end }}:\radius) {% if edge.weight %}node[dummy, midway, auto, sloped, text centered]{{'{$'}}{{edge.weight}}{{'$}'}}{% endif %};
{% endfor %}
